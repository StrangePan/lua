...
function identifier.is_valid(identifier_string)
  if type(identifier_string) ~= 'string' then
    return
      false,
      'not a valid util identifier: not of type \'string\' ('..type(identifier_string)..' received)'
  end
  ...
  if identifier_string_length < 1 then
    return
      false,
      'not a valid util identifier: identifiers must contain at least one character (empty string '
        ..'received)'
  end
  ...
  if not found_in(string.sub(identifier_string, 1, 1), valid_initial_chars) then
    return
      false,
      'not a valid util identifier: identifiers must begin with a letter (a-z or A-Z) or an '
  ...
  for i = 2,identifier_string_length do
    ...
    if not found_in(c, valid_initial_chars) and not found_in(c, valid_secondary_chars) then
      return
        false,
        'not a valid util identifier: identifiers can only contain letters (a-z or A-Z), '
